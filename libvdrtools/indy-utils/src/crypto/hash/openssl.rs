...
// these bytes are the same as openssl_hash(MessageDigest::sha256(), &[]) so we do not have to
// actually call the hash function
...
impl Hash {
    ...
    pub fn hash_leaf<T>(leaf: &T) -> Result<Vec<u8>, IndyError>
    where
        T: Hashable,
    {
    ...
    pub fn hash_nodes<T>(left: &T, right: &T) -> Result<Vec<u8>, IndyError>
    where
        T: Hashable,
    {
        let mut ctx = Hash::new_context()?;
