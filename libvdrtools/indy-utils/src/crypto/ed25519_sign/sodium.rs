...
pub fn create_key_pair_for_signature(seed: Option<&Seed>) -> Result<(PublicKey, SecretKey), IndyError> {
    let (public_key, secret_key) =
        sign::keypair_from_seed(&seed.unwrap_or(&Seed::from_slice(&randombytes(SEEDBYTES)).unwrap()).0);
...
mod tests {
    ...
    fn pk_to_curve25519_works() {
        let pk = vec![
            236, 191, 114, 144, 108, 87, 211, 244, 148, 23, 20, 175, 122, 6, 159, 254, 85, 99, 145, 152, 178, 133, 230,
            236, 192, 69, 35, 136, 141, 194, 243, 134,
        ];
        let pk = PublicKey::from_slice(&pk).unwrap();
        let pkc_test = vk_to_curve25519(&pk).unwrap();
        let pkc_exp = vec![
            8, 45, 124, 147, 248, 201, 112, 171, 11, 51, 29, 248, 34, 127, 197, 241, 60, 158, 84, 47, 4, 176, 238, 166,
            110, 39, 207, 58, 127, 110, 76, 42,
        ];
    ...
    fn sk_to_curve25519_works() {
        let sk = vec![
            78, 67, 205, 99, 150, 131, 75, 110, 56, 154, 76, 61, 27, 142, 36, 141, 44, 223, 122, 199, 14, 230, 12, 163,
            4, 255, 94, 230, 21, 242, 97, 200, 236, 191, 114, 144, 108, 87, 211, 244, 148, 23, 20, 175, 122, 6, 159,
            254, 85, 99, 145, 152, 178, 133, 230, 236, 192, 69, 35, 136, 141, 194, 243, 134,
        ];
        let sk = SecretKey::from_slice(&sk).unwrap();
        let skc_test = sk_to_curve25519(&sk).unwrap();
        let skc_exp = vec![
            144, 112, 64, 101, 69, 167, 61, 44, 220, 148, 58, 187, 108, 73, 11, 247, 130, 161, 158, 40, 100, 1, 40, 27,
            76, 148, 209, 240, 195, 35, 153, 121,
        ];
