...
pub use vdrtoolsrs::vdr::{VDRBuilder, VDR};
...
pub fn prepare_did(
    ...
    submitter_did: &str,
    endorser: Option<&str>,
) -> Result<(String, Vec<u8>, String, Vec<u8>, Option<String>), IndyError> {
...
pub fn prepare_cheqd_endorsement_data(
    ...
    vdr::prepare_cheqd_endorsement_data(
        vdr,
        wallet_handle,
        key_alias,
        did,
...
pub fn submit_raw_txn(vdr: &VDR, namespace: &str, txn_bytes: &[u8]) -> Result<String, IndyError> {
    vdr::submit_raw_txn(vdr, namespace, txn_bytes).wait()
}

pub fn submit_query(vdr: &VDR, namespace: &str, query: &str) -> Result<String, IndyError> {
