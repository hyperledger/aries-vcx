...
lazy_static! {
    static ref SCHEMA_RE: String = format!(
        "(did:indy(:{NAMESPACE_RE})?:{DID_RE}){SCHEMA_TYPE}({SCHEMA_NAME_RE})/({SCHEMA_VER_RE})"
    );
...
pub fn try_parse_indy_creddef_id(id: &str) -> Option<(String, String, String)> {
    ...
    let id_re = format!(
        "^(did:indy(:{NAMESPACE_RE})?:{DID_RE}){CREDDEF_TYPE}({schema_ref_re})/({CREDDEF_TAG_RE})$"
    );
    let id_re = Regex::new(id_re.as_str()).unwrap();
...
pub fn try_parse_indy_rev_reg(
    id: &str,
) -> Option<(DidValue, CredentialDefinitionId, String, String)> {
...
fn test_try_parse_valid_indy_revreg_works() {
    let (did, creddef, _, tag) = try_parse_indy_rev_reg(
        "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/REV_REG_DEF/1/creddef_name/TAG1",
    )
    .unwrap();
