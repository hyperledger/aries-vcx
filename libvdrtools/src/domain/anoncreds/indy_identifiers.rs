...
lazy_static! {
    static ref SCHEMA_RE: String = format!("(did:indy(:{NAMESPACE_RE})?:{DID_RE}){SCHEMA_TYPE}({SCHEMA_NAME_RE})/({SCHEMA_VER_RE})");
...
pub fn try_parse_indy_schema_id(id: &str) -> Option<(String, String, String)> {
    ...
    if let Some(captures) = id_re.captures(id) {
        ...
        if let (Some(did), Some(name), Some(ver))
        = (captures.get(1), captures.get(3), captures.get(4)) {
            return Some((did.as_str().to_owned(),
...
pub fn try_parse_indy_creddef_id(id: &str) -> Option<(String, String, String)> {
    ...
    if let Some(captures) = id_re.captures(id) {
        ...
        if let (Some(did), Some(seq_no), Some(tag))
        = (captures.get(1), captures.get(3), captures.get(9)) {
            return Some((did.as_str().to_owned(),
...
pub fn try_parse_indy_rev_reg(id: &str) -> Option<(DidValue, CredentialDefinitionId, String, String)> {
    ...
    let tag_re = r"[^/]*";
    let schema_ref_re = &*SCHEMA_REF_RE;
    let id_re = format!("^(did:indy(:{NAMESPACE_RE})?:{DID_RE})\
                                   /anoncreds/v0/REV_REG_DEF/{schema_ref_re}/({creddef_name_re})/({tag_re})$");
    let id_re = Regex::new(id_re.as_str()).unwrap();
...
fn test_try_parse_valid_indy_creddefid_works_for_sub_ledger() {
    let (did, schema_seq_no, tag) =
        try_parse_indy_creddef_id(
            "did:indy:sovrin:5nDyJVP1NrcPAttP3xwMB9/anoncreds/v0/CLAIM_DEF/56495/npdb").unwrap();
...
fn test_try_parse_valid_indy_creddefid_works() {
    let (did, schema_seq_no, tag) =
        try_parse_indy_creddef_id(
            "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/CLAIM_DEF/1/tag").unwrap();
    assert_eq!(did, "did:indy:NcYxiDXkpYi6ov5FcYDi1e".to_owned());
    assert_eq!(schema_seq_no, "1".to_owned());
    assert_eq!(tag, "tag".to_owned());

    let (did, schema_ref, tag) =
        try_parse_indy_creddef_id(
            "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/CLAIM_DEF/did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/SCHEMA/gvt/1.0/tag").unwrap();
    assert_eq!(did, "did:indy:NcYxiDXkpYi6ov5FcYDi1e".to_owned());
    assert_eq!(schema_ref, "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/SCHEMA/gvt/1.0".to_owned());
    assert_eq!(tag, "tag".to_owned());
}
...
fn test_try_parse_valid_indy_revreg_works() {
    let (did, creddef, _, tag) =
        try_parse_indy_rev_reg(
            "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/REV_REG_DEF/1/creddef_name/TAG1").unwrap();
    assert_eq!(did.0, "did:indy:NcYxiDXkpYi6ov5FcYDi1e".to_owned());
    assert_eq!(creddef.0, "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/CLAIM_DEF/1/creddef_name".to_owned());
    assert_eq!(tag, "TAG1".to_owned());

    let (did, creddef, _, tag) =
        try_parse_indy_rev_reg(
            "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/REV_REG_DEF/did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/SCHEMA/gvt/1.0/creddef_name/TAG1").unwrap();
    assert_eq!(did.0, "did:indy:NcYxiDXkpYi6ov5FcYDi1e".to_owned());
    assert_eq!(creddef.0, "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/CLAIM_DEF/did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/SCHEMA/gvt/1.0/creddef_name".to_owned());
    assert_eq!(tag, "TAG1".to_owned());
}
