...
pub fn try_parse_indy_rev_reg(
    ...
    let schema_ref_re = &*SCHEMA_REF_RE;
    let id_re = format!(
        "^(did:indy(:{NAMESPACE_RE})?:{DID_RE})/anoncreds/v0/REV_REG_DEF/{schema_ref_re}/\
         ({creddef_name_re})/({tag_re})$"
    );
    let id_re = Regex::new(id_re.as_str()).unwrap();
...
fn test_try_parse_valid_indy_creddefid_works() {
    ...
    let (did, schema_ref, tag) = try_parse_indy_creddef_id(
        "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/CLAIM_DEF/did:indy:NcYxiDXkpYi6ov5FcYDi1e/\
         anoncreds/v0/SCHEMA/gvt/1.0/tag",
    )
    .unwrap();
    assert_eq!(did, "did:indy:NcYxiDXkpYi6ov5FcYDi1e".to_owned());
    assert_eq!(
...
fn test_try_parse_valid_indy_revreg_works() {
    ...
    let (did, creddef, _, tag) = try_parse_indy_rev_reg(
        "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/REV_REG_DEF/did:indy:NcYxiDXkpYi6ov5FcYDi1e/\
         anoncreds/v0/SCHEMA/gvt/1.0/creddef_name/TAG1",
    )
    .unwrap();
    assert_eq!(did.0, "did:indy:NcYxiDXkpYi6ov5FcYDi1e".to_owned());
    assert_eq!(
        creddef.0,
        "did:indy:NcYxiDXkpYi6ov5FcYDi1e/anoncreds/v0/CLAIM_DEF/did:indy:NcYxiDXkpYi6ov5FcYDi1e/\
         anoncreds/v0/SCHEMA/gvt/1.0/creddef_name"
            .to_owned()
    );
    assert_eq!(tag, "TAG1".to_owned());
}
