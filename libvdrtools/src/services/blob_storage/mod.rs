...
impl BlobStorageService {
    pub(crate) async fn open_writer(&self, type_: &str, config: &str) -> IndyResult<i32> {
        let writer_config = self
            ...
            .await
            .get(type_)
            .ok_or_else(|| err_msg(IndyErrorKind::InvalidStructure, "Unknown BlobStorage Writer type"))?
            .open(config)
            .await?;
    ...
    pub(crate) async fn create_blob(&self, config_handle: i32) -> IndyResult<i32> {
        ...
        let writer = self
            ...
            .await
            .get(&config_handle)
            .ok_or_else(|| err_msg(IndyErrorKind::InvalidStructure, "Invalid BlobStorage config handle"))? // FIXME: Review error kind
            .create(blob_handle)
            .await?;
    ...
    pub(crate) async fn append(&self, handle: i32, bytes: &[u8]) -> IndyResult<usize> {
        ...
        let &mut (ref mut writer, ref mut hasher) = writers
            .get_mut(&handle)
            .ok_or_else(|| err_msg(IndyErrorKind::InvalidStructure, "Invalid BlobStorage handle"))?; // FIXME: Review error kind
    ...
    pub(crate) async fn finalize(&self, handle: i32) -> IndyResult<(String, Vec<u8>)> {
        ...
        writer.finalize(hash.as_slice()).await.map(|location| (location, hash))
    }
}
...
impl BlobStorageService {
    pub(crate) async fn open_reader(&self, type_: &str, config: &str) -> IndyResult<i32> {
        let reader_config = self
            ...
            .await
            .get(type_)
            .ok_or_else(|| err_msg(IndyErrorKind::InvalidStructure, "Invalid BlobStorage Reader type"))? // FIXME: Review error kind
            .open(config)
            .await?;
    ...
    pub(crate) async fn open_blob(&self, config_handle: i32, location: &str, hash: &[u8]) -> IndyResult<i32> {
        let reader = self
            .reader_configs
            .lock()
            .await
