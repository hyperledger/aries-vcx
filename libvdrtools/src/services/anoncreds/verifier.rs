...
use crate::{
    domain::anoncreds::{
        credential_definition::{CredentialDefinitionId, CredentialDefinitionV1},
        proof::{Identifier, Proof, RequestedProof, RevealedAttributeInfo},
        proof_request::{AttributeInfo, NonRevocedInterval, PredicateInfo, ProofRequestPayload},
        revocation_registry::RevocationRegistryV1,
        revocation_registry_definition::{RevocationRegistryDefinitionV1, RevocationRegistryId},
...
impl VerifierService {
    ...
    pub(crate) fn verify(
        ...
        VerifierService::_verify_revealed_attribute_values(&proof_req, &full_proof)?;

        VerifierService::_verify_requested_restrictions(
            &proof_req,
        ...
        VerifierService::_compare_timestamps_from_proof_and_request(
            proof_req,
            &received_revealed_attrs,
            &received_unrevealed_attrs,
            &received_self_attested_attrs,
        ...
        for sub_proof_index in 0..full_proof.identifiers.len() {
            ...
            let (rev_reg_def, rev_reg) = if let Some(timestamp) = identifier.timestamp {
                ...
                let rev_regs_for_cred = rev_regs.get(&rev_reg_id).ok_or_else(|| {
                    err_msg(
                        IndyErrorKind::InvalidStructure,
                        format!("RevocationRegistry not found for id: {:?}", rev_reg_id),
                    )
                })?;
            ...
            let attrs_for_credential = VerifierService::_get_revealed_attributes_for_credential(
                sub_proof_index,
                &full_proof.requested_proof,
                proof_req,
            )?;
            let predicates_for_credential = VerifierService::_get_predicates_for_credential(
                sub_proof_index,
                &full_proof.requested_proof,
                proof_req,
            )?;
    ...
    pub(crate) fn generate_nonce(&self) -> IndyResult<Nonce> {
    ...
    fn _compare_timestamps_from_proof_and_request(
        ...
        proof_req
            ...
            .iter()
            .map(|(referent, info)| {
                VerifierService::_validate_timestamp(
                    &received_revealed_attrs,
                    referent,
                ...
                .or_else(|_| {
                    received_self_attested_attrs
                        .get(referent)
                        .map(|_| ())
                        .ok_or_else(|| IndyError::from(IndyErrorKind::InvalidStructure))
                })
            })
    ...
    fn _validate_timestamp(
        ...
        if AnoncredsHelpers::get_non_revoc_interval(global_interval, local_interval).is_none() {
            return Ok(());
        }
    ...
    fn _received_revealed_attrs(proof: &Proof) -> IndyResult<HashMap<String, Identifier>> {
        ...
        for (referent, info) in proof.requested_proof.revealed_attrs.iter() {
            revealed_identifiers.insert(
                referent.to_string(),
                VerifierService::_get_proof_identifier(proof, info.sub_proof_index)?,
            );
        }
        for (referent, infos) in proof.requested_proof.revealed_attr_groups.iter() {
            revealed_identifiers.insert(
                referent.to_string(),
                VerifierService::_get_proof_identifier(proof, infos.sub_proof_index)?,
            );
        }
    ...
    fn _verify_revealed_attribute_values(proof_req: &ProofRequestPayload, proof: &Proof) -> IndyResult<()> {
        ...
        for (attr_referent, attr_infos) in proof.requested_proof.revealed_attr_groups.iter() {
            ...
            for attr_name in attr_names {
                let attr_info = &attr_infos.values.get(attr_name).ok_or(IndyError::from_msg(
                    IndyErrorKind::InvalidStructure,
                    "Proof Revealed Attr Group does not match Proof Request Attribute Group",
                ))?;
    ...
    fn _verify_requested_restrictions(
        ...
        for (referent, info) in proof_req.requested_predicates.iter() {
            if let Some(ref query) = info.restrictions {
                let filter = VerifierService::_gather_filter_info(&referent, received_predicates)?;
    ...
    fn _process_operator(
        ...
        VerifierService::_do_process_operator(&attr_value_map, restriction_op, filter)
    }
    ...
    fn _do_process_operator(
        ...
        match restriction_op {
            ...
            Query::And(ref operators) => operators
                .iter()
                .map(|op| VerifierService::_do_process_operator(attr_value_map, op, filter))
                .collect::<IndyResult<Vec<()>>>()
                .map(|_| ())
    ...
    fn _process_filter(
        ...
        match tag {
            ...
            x if VerifierService::_is_attr_marker_operator(x) => {
    ...
    pub(crate) fn attr_request_by_value(key: &str) -> Option<&str> {
        VALUE_TAG_MATCHER
            .captures(key)
            .and_then(|caps| caps.get(1).map(|s| s.as_str()))
    }

    pub(crate) fn attr_request_by_marker(key: &str) -> Option<&str> {
        MARKER_TAG_MATCHER
            .captures(key)
            .and_then(|caps| caps.get(1).map(|s| s.as_str()))
    }
...
mod tests {
    ...
    pub(crate) const SCHEMA_ID: &str = "123";
    pub(crate) const SCHEMA_NAME: &str = "Schema Name";
    pub(crate) const SCHEMA_ISSUER_DID: &str = "234";
    pub(crate) const SCHEMA_VERSION: &str = "1.2.3";
    pub(crate) const CRED_DEF_ID: &str = "345";
    pub(crate) const ISSUER_DID: &str = "456";
    ...
    fn test_process_op_eq_revealed_value() {
        ...
        VerifierService::_process_operator("zip", &op, &filter, Some(value)).unwrap();
        ...
        assert!(VerifierService::_process_operator("zip", &op, &filter, Some("NOT HERE")).is_err());
    }
    ...
    fn test_process_op_eq_revealed_value_case_insensitive() {
        ...
        VerifierService::_process_operator("Given Name", &op, &filter, Some(value)).unwrap();
