...
lazy_static! {
    ...
    static ref POOL_HANDLE_SENDERS: Arc<Mutex<HashMap<PoolHandle, oneshot::Sender<IndyResult<(PoolHandle, String)>>>>> = Arc::new(Mutex::new(HashMap::new()));
...
impl PoolService {
    ...
    pub(crate) async fn open(&self, name: String, config: Option<PoolOpenConfig>, handle: Option<PoolHandle>) -> IndyResult<(PoolHandle, String)> {
        ...
        let pool_handle: PoolHandle = handle.unwrap_or_else(|| next_pool_handle());
        ...
        let (sender, receiver) = oneshot::channel::<IndyResult<(PoolHandle, String)>>();
    ...
    //#[logfn(trace)] FIXME:
    pub(crate) async fn refresh_ack(cmd_id: CommandHandle, result: IndyResult<String>) {
    ...
    pub(crate) async fn refresh(&self, handle: PoolHandle) -> IndyResult<String> {
        self.send_action(handle, COMMAND_REFRESH, None, None).await
    }
...
pub mod test_utils {
    ...
    pub(crate) async fn fake_pool_handle_for_poolsm() -> (
        ...
        oneshot::Receiver<IndyResult<(PoolHandle, String)>>,
    ) {
    ...
    mod pool_service {
        ...
        async fn pool_service_refresh_works() {
            ...
            let pool_mock = thread::spawn(move || {
                ...
                block_on(PoolService::refresh_ack(cmd_id, Ok(String::from("stub"))));
            });
