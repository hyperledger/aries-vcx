...
impl<T: Networker> RequestSM<T> {
    fn handle_event(self, re: RequestEvent) -> (Self, Option<PoolEvent>) {
        let RequestSM { state, f, cmd_ids, nodes, generator, pool_name, timeout, extended_timeout, number_read_nodes, pool_mode } = self;
                ...
                match re {
                    RequestEvent::LedgerStatus(ls, Some(node_alias), _) => {
                        RequestSM::_catchup_target_handle_consensus_state(
                            ...
                            node_alias, ls.merkleRoot, f, &nodes, &pool_name, pool_mode)
                    }
                    RequestEvent::ConsistencyProof(cp, node_alias) => {
                        RequestSM::_catchup_target_handle_consensus_state(
                            state,
                            cp.newMerkleRoot, cp.seqNoEnd, Some(cp.hashes),
                            node_alias, cp.oldMerkleRoot, f, &nodes, &pool_name, pool_mode)
                    }
                    RequestEvent::Timeout(req_id, node_alias) => {
                        RequestSM::_catchup_target_handle_consensus_state(
                            state,
                            "timeout".to_string(), 0, None,
                            node_alias, req_id, f, &nodes, &pool_name, pool_mode)
                    }
            ...
            RequestState::CatchupSingle(state) => {
                match re {
                    RequestEvent::CatchupRep(mut cr, node_alias) => {
                        match _process_catchup_reply(&mut cr, &state.merkle_tree, &state.target_mt_root, state.target_mt_size, &pool_name,  pool_mode) {
                            Ok(merkle) => {
        ...
        (RequestSM::step(f, cmd_ids, nodes, generator, pool_name, timeout, extended_timeout, number_read_nodes, pool_mode, state), event)
    }
    ...
    fn _catchup_target_handle_consensus_state(mut state: CatchupConsensusState<T>,
                                              ...
                                              f: usize, nodes: &Nodes,
                                              pool_name: &str,
                                              pool_mode: PoolMode) -> (RequestState<T>, Option<PoolEvent>) {
        let (finished, result) = RequestSM::_process_catchup_target(mt_root, sz, cons_proof,
                                                                    &node_alias, &mut state, f, nodes, pool_name, pool_mode);
    ...
    fn _process_catchup_target(merkle_root: String,
                               ...
                               nodes: &Nodes,
                               pool_name: &str,
                               pool_mode: PoolMode) -> (bool, Option<PoolEvent>) {
...
pub trait RequestHandler<T: Networker> {
    fn new(networker: Arc<Mutex<T>>, f: usize, cmd_ids: &[CommandHandle], nodes: &Nodes, pool_name: &str, timeout: i64, extended_timeout: i64, number_read_nodes: u8, pool_mode: PoolMode) -> Self;
...
impl<T: Networker> RequestHandler<T> for RequestHandlerImpl<T> {
    fn new(networker: Arc<Mutex<T>>, f: usize, cmd_ids: &[CommandHandle], nodes: &Nodes, pool_name: &str, timeout: i64, extended_timeout: i64, number_read_nodes: u8, pool_mode: PoolMode) -> Self {
        RequestHandlerImpl {
            request_wrapper: Some(RequestSM::new(networker, f, cmd_ids, nodes, pool_name, timeout, extended_timeout, number_read_nodes, pool_mode)),
        }
    }
...
fn _process_catchup_reply(rep: &mut CatchupRep, merkle: &MerkleTree, target_mt_root: &Vec<u8>, target_mt_size: usize, pool_name: &str, pool_mode: PoolMode) -> IndyResult<MerkleTree> {
...
pub mod tests {
    ...
    impl<T: Networker> RequestHandler<T> for MockRequestHandler {
        fn new(_networker: Arc<Mutex<T>>, _f: usize, _cmd_ids: &[CommandHandle], _nodes: &Nodes, _pool_name: &str, _timeout: i64, _extended_timeout: i64, _number_read_nodes: u8, _pool_mode: PoolMode) -> Self {
            MockRequestHandler {}
        }
