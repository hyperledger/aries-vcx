...
pub fn check_nodes_responses_on_status(
    ...
    if let Some((most_popular_not_timeout_vote, votes_cnt)) = most_popular_not_timeout {
        ...
        if *votes_cnt == node_cnt - f {
            return _reset_cache_and_restart_catch_up(
                pool_name,
                pool_mode,
...
fn _if_consensus_reachable(
    ...
    if is_consensus_not_reachable {
        ...
        _reset_cache_and_restart_catch_up(
            pool_name,
            pool_mode,
            err_msg(IndyErrorKind::PoolTimeout, "No consensus possible"),
        )
    } else {
        Ok(CatchupProgress::InProgress)
    }
...
fn _reset_cache_and_restart_catch_up(
    pool_name: &str,
    pool_mode: PoolMode,
    err: IndyError,
) -> IndyResult<CatchupProgress> {
...
fn _try_to_catch_up(
    ...
    if target_mt_size == cur_mt_size {
        ...
        let target_mt_root = target_mt_root.from_base58().map_err(Context::new).to_indy(
