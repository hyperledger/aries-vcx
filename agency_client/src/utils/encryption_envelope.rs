...
impl EncryptionEnvelope {
    async fn _unpack_a2a_message(
        ...
        trace!(
            "EncryptionEnvelope::_unpack_a2a_message >>> msg_value: {:?}",
            msg_value
        );

        let sender_vk = msg_value["sender_verkey"].as_str().map(String::from);
        trace!(
            "EncryptionEnvelope::_unpack_a2a_message >>> sender_vk: {:?}",
            sender_vk
        );
    ...
    pub async fn anon_unpack(
        ...
        if AgencyMockDecrypted::has_decrypted_mock_messages() {
            ...
            trace!(
                "EncryptionEnvelope::anon_unpack >>> a2a_message: {:?}",
                a2a_message
            );
            Ok(a2a_message)
        }
    ...
    pub async fn auth_unpack(
        ...
        if AgencyMockDecrypted::has_decrypted_mock_messages() {
            ...
            trace!(
                "EncryptionEnvelope::auth_unpack >>> a2a_message: {:?}",
                a2a_message
            );
            Ok(a2a_message)
        }
