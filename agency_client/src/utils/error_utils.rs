...
pub fn kind_to_error_message(kind: &AgencyClientErrorKind) -> String {
    match kind {
        ...
        AgencyClientErrorKind::LibindyInvalidStructure => {
            "Object (json, config, key, credential and etc...) passed to libindy has invalid structure".into()
        }
        ...
        AgencyClientErrorKind::DuplicationMasterSecret => {
            "Attempted to add a Master Secret that already existed in wallet".into()
        }
        AgencyClientErrorKind::DuplicationDid => {
            "Attempted to add a DID to wallet when that DID already exists in wallet".into()
        }
