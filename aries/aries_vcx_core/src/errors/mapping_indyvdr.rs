...
impl From<VdrError> for AriesVcxCoreError {
    fn from(err: VdrError) -> Self {
        match err.kind() {
            VdrErrorKind::Config => {
                AriesVcxCoreError::from_msg(AriesVcxCoreErrorKind::InvalidConfiguration, err)
            }
            ...
            VdrErrorKind::FileSystem(_) => {
                AriesVcxCoreError::from_msg(AriesVcxCoreErrorKind::IOError, err)
            }
            VdrErrorKind::Input => {
