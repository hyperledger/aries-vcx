...
mod tests {
    ...
    fn test_box_should_encrypt_and_decrypt() {
        ...
        let sender_private_bytes = &sender_key.to_secret_bytes().unwrap();
        let recipient_key = LocalKey::generate(X25519, true).unwrap();
        let recipient_public_bytes = &recipient_key.to_public_bytes().unwrap();
        let msg = "secret";
        ...
        let (enc, nonce) = crypto_box
            ...
            .unwrap();
        let recipient_private_bytes = &recipient_key.to_secret_bytes().unwrap();
        let sender_public_bytes = &sender_key.to_public_bytes().unwrap();
        let res = crypto_box
