...
impl RecordCategory {
    pub fn from_str(value: &str) -> VcxCoreResult<Self> {
        match value {
            LINK_SECRET => Ok(RecordCategory::LinkSecret),
            CRED => Ok(RecordCategory::Cred),
            CRED_DEF => Ok(RecordCategory::CredDef),
            CRED_KEY_CORRECTNESS_PROOF => Ok(RecordCategory::CredKeyCorectnessProof),
...
impl Display for RecordCategory {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let value = match self {
            ...
            RecordCategory::CredKeyCorectnessProof => CRED_KEY_CORRECTNESS_PROOF,
