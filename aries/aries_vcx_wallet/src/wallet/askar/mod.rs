...
impl AskarWallet {
        ...
        let maybe_entry = session
            ...
            .map_err(|err| match err.kind() {
                aries_askar::ErrorKind::NotFound => {
                    VcxWalletError::record_not_found(category, name)
                }
                _ => err.into(),
            })?;

        maybe_entry.ok_or_else(|| VcxWalletError::record_not_found(category, name))
    }
    ...
    async fn fetch_key_entry(
        ...
        session
            ...
            .await?
            .ok_or_else(|| VcxWalletError::record_not_found(RecordCategory::Key, key_name))
    }
