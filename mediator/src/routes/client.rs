...
pub async fn handle_register(
    ...
    let (state, EncryptionEnvelope(packed_aries_msg_bytes)) =
        agent.gen_client_connect_req(oob_invite.clone()).await?;
    let packed_aries_msg_json: Value = serde_json::from_slice(&packed_aries_msg_bytes[..])
        .expect("Envelope content should be serializable json");
    info!(
        "Sending Connection Request Envelope: {},",
        serde_json::to_string_pretty(&packed_aries_msg_json).unwrap()
    );
    let oob_invited_endpoint = oob2did(oob_invite)
        .get_endpoint()
    ...
    let res_aries: AriesMessage =
        serde_json::from_str(&res_unpack.message).map_err(|err| err.to_string())?;
