...
use aries_vcx::aries_vcx_core::wallet::internal::{close_search_wallet, fetch_next_records_wallet, open_search_wallet};
...
use crate::api_vcx::api_global::profile::{
    get_main_anoncreds, get_main_indy_ledger_write, get_main_profile, get_main_wallet, try_get_main_wallet,
};
...
pub async fn rotate_verkey_apply(did: &str, temp_vk: &str) -> LibvcxResult<()> {
    map_ariesvcx_result(
        aries_vcx::common::keys::rotate_verkey_apply(&get_main_wallet()?, &get_main_indy_ledger_write()?, did, temp_vk)
            .await,
    )
}
