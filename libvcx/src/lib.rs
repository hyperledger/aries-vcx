...
mod tests {
    ...
    use super::*;
    use crate::utils::devsetup_agent::test::{TestAgent, Alice, Faber};
    ...
    fn create_and_send_cred_offer(faber: &Faber, did: &str, cred_def_handle: u32, connection: u32, credential_data: &str, comment: Option<&str>) -> u32 {
        faber.activate();
    ...
    fn send_cred_req(alice: &Alice, connection: u32, comment: Option<&str>) -> u32 {
    ...
    fn send_credential(consumer: &Alice, institution: &Faber, handle_issuer_credential: u32, issuer_to_consumer: u32, consumer_to_issuer: u32, handle_holder_credential: u32, revokable: bool) {
    ...
    fn send_proof_request(faber: &Faber, connection_handle: u32, requested_attrs: &str, requested_preds: &str, revocation_interval: &str, request_name: Option<&str>) -> u32 {
        faber.activate();
    ...
    fn create_proof(alice: &Alice, connection_handle: u32, request_name: Option<&str>) -> u32 {
        alice.activate();
    ...
    fn generate_and_send_proof(alice: &Alice, proof_handle: u32, connection_handle: u32, selected_credentials: &str) {
        alice.activate();
    ...
    fn revoke_credential(faber: &Faber, credential_handle: u32, rev_reg_id: Option<String>) {
        faber.activate();
    ...
    fn revoke_credential_local(faber: &Faber, issuer_credential_handle: u32, rev_reg_id: Option<String>) {
        faber.activate();
        let (_, delta, timestamp) = libindy::utils::anoncreds::get_rev_reg_delta_json(&rev_reg_id.clone().unwrap(), None, None).unwrap();
    ...
    fn rotate_rev_reg(faber: &Faber, cred_def_handle: u32) {
        faber.activate();
    ...
    fn publish_revocation(institution: &Faber, rev_reg_id: String) {
        institution.activate();
    ...
    fn _exchange_credential(consumer: &Alice, institution: &Faber, credential_data: String, cred_def_handle: u32, consumer_to_issuer: u32, issuer_to_consumer: u32, comment: Option<&str>) -> u32 {
    ...
    fn _issue_address_credential(consumer: &Alice, institution: &Faber, consumer_to_institution: u32, institution_to_consumer: u32) -> (String, String, Option<String>, u32, u32) {
    ...
    fn _verifier_create_proof_and_send_request(institution: &Faber, institution_to_consumer:u32, schema_id: &str, cred_def_id: &str, request_name: Option<&str>) -> u32 {
        institution.activate();
    ...
    fn _prover_select_credentials_and_send_proof(consumer: &Alice, consumer_to_institution: u32, request_name: Option<&str>, requested_values: Option<&str>) {
        consumer.activate();
    ...
    fn test_basic_revocation() {
        ...
        let (consumer_to_institution, institution_to_consumer) = connection::tests::create_connected_connections(&consumer, &institution);
        let (schema_id, cred_def_id, rev_reg_id, _cred_def_handle, credential_handle) = _issue_address_credential(&consumer, &institution, consumer_to_institution, institution_to_consumer);

        let time_before_revocation = time::get_time().sec as u64;
        info!("test_basic_revocation :: verifier :: Going to revoke credential");
        revoke_credential(&institution, credential_handle, rev_reg_id);
        thread::sleep(Duration::from_millis(2000));
        ...
        let proof_handle_verifier = send_proof_request(&institution, institution_to_consumer, &requested_attrs_string, "[]", &interval, None);

        _prover_select_credentials_and_send_proof(&consumer, consumer_to_institution, None, None);
    ...
    fn test_local_revocation() {
        ...
        let (consumer_to_institution, institution_to_consumer) = connection::tests::create_connected_connections(&consumer, &institution);
        let (schema_id, cred_def_id, rev_reg_id, _cred_def_handle, issuer_credential_handle) = _issue_address_credential(&consumer, &institution, consumer_to_institution, institution_to_consumer);

        revoke_credential_local(&institution, issuer_credential_handle, rev_reg_id.clone());
        let request_name1 = Some("request1");
        let proof_handle_verifier = _verifier_create_proof_and_send_request(&institution, institution_to_consumer, &schema_id, &cred_def_id, request_name1);
        _prover_select_credentials_and_send_proof(&consumer, consumer_to_institution, request_name1, None);

        institution.activate();
        proof::update_state(proof_handle_verifier, None, institution_to_consumer).unwrap();
        assert_eq!(proof::get_proof_state(proof_handle_verifier).unwrap(), ProofStateType::ProofValidated as u32);

        publish_revocation(&institution, rev_reg_id.clone().unwrap());
        let request_name2 = Some("request2");
        let proof_handle_verifier = _verifier_create_proof_and_send_request(&institution, institution_to_consumer, &schema_id, &cred_def_id, request_name2);
        _prover_select_credentials_and_send_proof(&consumer, consumer_to_institution, request_name2, None);
    ...
    fn test_double_issuance_separate_issuer_and_consumers() {
        ...
        let verifier = Faber::setup();
        let consumer1 = Alice::setup();
        let consumer2 = Alice::setup();
        let (consumer1_to_verifier, verifier_to_consumer1) = connection::tests::create_connected_connections(&consumer1, &verifier);
        let (consumer1_to_issuer, issuer_to_consumer1) = connection::tests::create_connected_connections(&consumer1, &issuer);
        let (consumer2_to_verifier, verifier_to_consumer2) = connection::tests::create_connected_connections(&consumer2, &verifier);
        let (consumer2_to_issuer, issuer_to_consumer2) = connection::tests::create_connected_connections(&consumer2, &issuer);
        ...
        let _credential_handle1 = _exchange_credential(&consumer1, &issuer, credential_data1, cred_def_handle, consumer1_to_issuer, issuer_to_consumer1, None);
        ...
        let _credential_handle2 = _exchange_credential(&consumer2, &issuer, credential_data2, cred_def_handle,  consumer2_to_issuer, issuer_to_consumer2, None);

        let request_name1 = Some("request1");
        let proof_handle_verifier = _verifier_create_proof_and_send_request(&verifier, verifier_to_consumer1, &schema_id, &cred_def_id, request_name1);
        _prover_select_credentials_and_send_proof(&consumer1, consumer1_to_verifier, None, None);
        verifier.activate();
        proof::update_state(proof_handle_verifier, None, verifier_to_consumer1).unwrap();
        assert_eq!(proof::get_proof_state(proof_handle_verifier).unwrap(), ProofStateType::ProofValidated as u32);

        let request_name2 = Some("request2");
        let proof_handle_verifier = _verifier_create_proof_and_send_request(&verifier, verifier_to_consumer2, &schema_id, &cred_def_id, request_name2);
        _prover_select_credentials_and_send_proof(&consumer2, consumer2_to_verifier, None, None);
        verifier.activate();
    ...
    fn test_double_issuance_separate_issuer() {
        ...
        let (consumer_to_verifier, verifier_to_consumer) = connection::tests::create_connected_connections(&consumer, &verifier);
        let (consumer_to_issuer, issuer_to_consumer) = connection::tests::create_connected_connections(&consumer, &issuer);

        let (schema_id, cred_def_id, _rev_reg_id, _cred_def_handle, _credential_handle) = _issue_address_credential(&consumer, &issuer, consumer_to_issuer, issuer_to_consumer);
        issuer.activate();
        let request_name1 = Some("request1");
        let proof_handle_verifier = _verifier_create_proof_and_send_request(&verifier, verifier_to_consumer, &schema_id, &cred_def_id,  request_name1);
        _prover_select_credentials_and_send_proof(&consumer, consumer_to_verifier, request_name1, None);
        verifier.activate();
        proof::update_state(proof_handle_verifier, None, verifier_to_consumer).unwrap();
        assert_eq!(proof::get_proof_state(proof_handle_verifier).unwrap(), ProofStateType::ProofValidated as u32);

        let request_name2 = Some("request2");
        let proof_handle_verifier = _verifier_create_proof_and_send_request(&verifier, verifier_to_consumer, &schema_id, &cred_def_id, request_name2);
        _prover_select_credentials_and_send_proof(&consumer, consumer_to_verifier, request_name2, None);
        verifier.activate();
        proof::update_state(proof_handle_verifier, None, verifier_to_consumer).unwrap();
        assert_eq!(proof::get_proof_state(proof_handle_verifier).unwrap(), ProofStateType::ProofValidated as u32);
    }
    ...
    fn test_double_issuance_issuer_is_verifier() {
        ...
        let _credential_handle = _exchange_credential(&consumer, &institution, credential_data, cred_def_handle, consumer_to_institution, institution_to_consumer, None);
    ...
    fn test_batch_revocation() {
        ...
        let consumer1 = Alice::setup();
        let consumer2 = Alice::setup();
        let consumer3 = Alice::setup();
        let (consumer_to_institution1, institution_to_consumer1) = connection::tests::create_connected_connections(&consumer1, &institution);
        let (consumer_to_institution2, institution_to_consumer2) = connection::tests::create_connected_connections(&consumer2, &institution);
        let (consumer_to_institution3, institution_to_consumer3) = connection::tests::create_connected_connections(&consumer3, &institution);
        ...
        let (address1, address2, city, state, zip) = attr_names();
        let credential_data1 = json!({address1.clone(): "123 Main St", address2.clone(): "Suite 3", city.clone(): "Draper", state.clone(): "UT", zip.clone(): "84000"}).to_string();
        let credential_handle1 = _exchange_credential(&consumer1, &institution, credential_data1, cred_def_handle, consumer_to_institution1, institution_to_consumer1, None);
        let credential_data2 = json!({address1.clone(): "101 Tela Lane", address2.clone(): "Suite 1", city.clone(): "SLC", state.clone(): "WA", zip.clone(): "8721"}).to_string();
        let credential_handle2 = _exchange_credential(&consumer2, &institution, credential_data2, cred_def_handle, consumer_to_institution2, institution_to_consumer2, None);
        ...
        let credential_handle3 = _exchange_credential(&consumer3, &institution, credential_data3, cred_def_handle, consumer_to_institution3, institution_to_consumer3, None);

        revoke_credential_local(&institution,credential_handle1, rev_reg_id.clone());
        revoke_credential_local(&institution,credential_handle2, rev_reg_id.clone());

        // Revoke two locally and verify their are all still valid
        let request_name1 = Some("request1");
        let proof_handle_verifier1 = _verifier_create_proof_and_send_request(&institution, institution_to_consumer1, &schema_id, &cred_def_id, request_name1);
        _prover_select_credentials_and_send_proof(&consumer1, consumer_to_institution1, request_name1, None);
        let proof_handle_verifier2 = _verifier_create_proof_and_send_request(&institution, institution_to_consumer2, &schema_id, &cred_def_id, request_name1);
        _prover_select_credentials_and_send_proof(&consumer2, consumer_to_institution2, request_name1, None);
        let proof_handle_verifier3 = _verifier_create_proof_and_send_request(&institution, institution_to_consumer3, &schema_id, &cred_def_id, request_name1);
        _prover_select_credentials_and_send_proof(&consumer3, consumer_to_institution3, request_name1, None);
        ...
        thread::sleep(Duration::from_millis(2000));
        let request_name2 = Some("request2");
        let proof_handle_verifier1 = _verifier_create_proof_and_send_request(&institution, institution_to_consumer1, &schema_id, &cred_def_id, request_name2);
        _prover_select_credentials_and_send_proof(&consumer1, consumer_to_institution1,  request_name2, None);
        let proof_handle_verifier2 = _verifier_create_proof_and_send_request(&institution, institution_to_consumer2, &schema_id, &cred_def_id, request_name2);
        _prover_select_credentials_and_send_proof(&consumer2, consumer_to_institution2,  request_name2, None);
        let proof_handle_verifier3 = _verifier_create_proof_and_send_request(&institution, institution_to_consumer3, &schema_id, &cred_def_id, request_name2);
        _prover_select_credentials_and_send_proof(&consumer3, consumer_to_institution3,  request_name2, None);
    ...
    fn test_revoked_credential_might_still_work() {
        ...
        let proof_handle_prover = create_proof(&consumer, consumer_to_institution, None);
    ...
    fn test_real_proof() {
        ...
        let (consumer_to_issuer, issuer_to_consumer) = connection::tests::create_connected_connections(&consumer, &institution);
        ...
        let institution_did = settings::get_config_value(settings::CONFIG_INSTITUTION_DID).unwrap();
        let handle_issuer_credential = create_and_send_cred_offer(&institution, &institution_did, cred_def_handle, issuer_to_consumer, &credential_data, None);

        info!("test_real_proof :: AS CONSUMER SEND CREDENTIAL REQUEST");
        let handle_holder_credential = send_cred_req(&consumer, consumer_to_issuer, None);

        info!("test_real_proof :: AS INSTITUTION SEND CREDENTIAL");
        send_credential(&consumer, &institution, handle_issuer_credential, issuer_to_consumer, consumer_to_issuer, handle_holder_credential, false);
        ...
        let proof_req_handle = send_proof_request(&institution, issuer_to_consumer, &requested_attrs, "[]", "{}", None);

        info!("test_real_proof :: Going to create proof");
        let proof_handle = create_proof(&consumer, consumer_to_issuer, None);
        ...
        generate_and_send_proof(&consumer, proof_handle, consumer_to_issuer, &serde_json::to_string(&selected_credentials).unwrap());
    ...
    fn test_two_creds_one_rev_reg() {
        ...
        let credential_data1 = json!({address1.clone(): "123 Main St", address2.clone(): "Suite 3", city.clone(): "Draper", state.clone(): "UT", zip.clone(): "84000"}).to_string();
        let _credential_handle1 = _exchange_credential(&consumer, &issuer, credential_data1.clone(), cred_def_handle, consumer_to_issuer, issuer_to_consumer, req1);
        let credential_data2 = json!({address1.clone(): "101 Tela Lane", address2.clone(): "Suite 1", city.clone(): "SLC", state.clone(): "WA", zip.clone(): "8721"}).to_string();
        let _credential_handle2 = _exchange_credential(&consumer, &issuer,credential_data2.clone(), cred_def_handle, consumer_to_issuer, issuer_to_consumer, req2);

        let proof_handle_verifier = _verifier_create_proof_and_send_request(&verifier, verifier_to_consumer, &schema_id, &cred_def_id, req1);
        _prover_select_credentials_and_send_proof(&consumer, consumer_to_verifier, req1, Some(&credential_data1));
        verifier.activate();
        proof::update_state(proof_handle_verifier, None, verifier_to_consumer).unwrap();
        assert_eq!(proof::get_proof_state(proof_handle_verifier).unwrap(), ProofStateType::ProofValidated as u32);

        let proof_handle_verifier = _verifier_create_proof_and_send_request(&verifier, verifier_to_consumer, &schema_id, &cred_def_id, req2);
        _prover_select_credentials_and_send_proof(&consumer, consumer_to_verifier, req2, Some(&credential_data2));
        verifier.activate();
        proof::update_state(proof_handle_verifier, None, verifier_to_consumer).unwrap();
        assert_eq!(proof::get_proof_state(proof_handle_verifier).unwrap(), ProofStateType::ProofValidated as u32);
    }
    ...
    fn test_two_creds_one_rev_reg_revoke_first() {
        ...
        let (req1, req2) = (Some("request1"), Some("request2"));
        let credential_data1 = json!({address1.clone(): "123 Main St", address2.clone(): "Suite 3", city.clone(): "Draper", state.clone(): "UT", zip.clone(): "84000"}).to_string();
        let credential_handle1 = _exchange_credential(&consumer, &issuer, credential_data1.clone(), cred_def_handle, consumer_to_issuer, issuer_to_consumer,  req1);
        let credential_data2 = json!({address1.clone(): "101 Tela Lane", address2.clone(): "Suite 1", city.clone(): "SLC", state.clone(): "WA", zip.clone(): "8721"}).to_string();
        let _credential_handle2 = _exchange_credential(&consumer, &issuer, credential_data2.clone(), cred_def_handle, consumer_to_issuer, issuer_to_consumer, req2);

        revoke_credential(&issuer, credential_handle1, rev_reg_id);
    ...
    fn test_two_creds_one_rev_reg_revoke_second() {
        ...
        let _credential_handle1 = _exchange_credential(&consumer, &issuer, credential_data1.clone(), cred_def_handle, consumer_to_issuer, issuer_to_consumer,  req1);
        let credential_data2 = json!({address1.clone(): "101 Tela Lane", address2.clone(): "Suite 1", city.clone(): "SLC", state.clone(): "WA", zip.clone(): "8721"}).to_string();
        let credential_handle2 = _exchange_credential(&consumer, &issuer, credential_data2.clone(), cred_def_handle, consumer_to_issuer, issuer_to_consumer, req2);

        revoke_credential(&issuer, credential_handle2, rev_reg_id);
    ...
    fn test_two_creds_two_rev_reg_id() {
        ...
        let credential_data1 = json!({address1.clone(): "123 Main St", address2.clone(): "Suite 3", city.clone(): "Draper", state.clone(): "UT", zip.clone(): "84000"}).to_string();
        let credential_handle1 = _exchange_credential(&consumer, &issuer, credential_data1.clone(), cred_def_handle, consumer_to_issuer, issuer_to_consumer,  req1);
        rotate_rev_reg(&issuer, cred_def_handle);
        let credential_data2 = json!({address1.clone(): "101 Tela Lane", address2.clone(): "Suite 1", city.clone(): "SLC", state.clone(): "WA", zip.clone(): "8721"}).to_string();
        let _credential_handle2 = _exchange_credential(&consumer, &issuer, credential_data2.clone(), cred_def_handle, consumer_to_issuer, issuer_to_consumer, req2);
