    ...
    execute(move || {
        match credential::get_attributes(credential_handle) {
            Ok(s) => {
                ...
                cb(command_handle, libvcx_error::SUCCESS.code_num, attrs.as_ptr());
            }
            Err(err) => {
                set_current_error_vcx(&err);
                error!(
    ...
    execute(move || {
        match credential::get_attachment(credential_handle) {
            Ok(s) => {
                ...
                cb(command_handle, libvcx_error::SUCCESS.code_num, attach.as_ptr());
            }
            Err(err) => {
                set_current_error_vcx(&err);
                error!(
    ...
    execute(move || {
        match credential::get_tails_location(credential_handle) {
            Ok(s) => {
                ...
                cb(command_handle, libvcx_error::SUCCESS.code_num, location.as_ptr());
            }
            Err(err) => {
                set_current_error_vcx(&err);
                error!(
    ...
    execute(move || {
        match credential::get_tails_hash(credential_handle) {
            Ok(s) => {
                ...
                cb(command_handle, libvcx_error::SUCCESS.code_num, hash.as_ptr());
            }
            Err(err) => {
                set_current_error_vcx(&err);
                error!(
    ...
    execute_async::<BoxFuture<'static, Result<(), ()>>>(Box::pin(async move {
        match credential::credential_create_with_msgid(&source_id, connection_handle, &msg_id).await {
            Ok((handle, offer_string)) => {
                ...
                trace!("vcx_credential_create_with_offer_cb(command_handle: {}, source_id: {}, rc: {}, handle: {}) source_id: {}", command_handle, source_id, libvcx_error::SUCCESS.message, handle, source_id);
                cb(command_handle, libvcx_error::SUCCESS.code_num, handle, c_offer.as_ptr())
            }
            Err(err) => {
    ...
    execute_async::<BoxFuture<'static, Result<(), ()>>>(Box::pin(async move {
        ...
        match credential::get_state(credential_handle) {
            Ok(s) => {
                ...
                cb(command_handle, libvcx_error::SUCCESS.code_num, s)
            }
            Err(err) => {
                error!(
                    "vcx_v2_credential_update_state_cb(command_handle: {}, rc: {}, state: {}), source_id: {:?}",
...
pub extern "C" fn vcx_v2_credential_update_state_with_message(
    ...
    execute_async::<BoxFuture<'static, Result<(), ()>>>(Box::pin(async move {
        ...
        match credential::get_state(credential_handle) {
            Ok(s) => {
                trace!("vcx_v2_credential_update_state_with_message_cb(command_handle: {}, rc: {}, state: {}), source_id: {:?}", command_handle, libvcx_error::SUCCESS.message, s, source_id);
                cb(command_handle, libvcx_error::SUCCESS.code_num, s)
            }
            Err(err) => {
                error!("vcx_v2_credential_update_state_with_message_cb(command_handle: {}, rc: {}, state: {}), source_id: {:?}", command_handle, err, 0, source_id);
...
mod tests {
    ...
    async fn test_vcx_credential_get_new_offers() {
        ...
        assert_eq!(
            ...
            libvcx_error::SUCCESS.code_num as u32
        );
    ...
    fn test_vcx_credential_release() {
        ...
        assert_eq!(vcx_credential_release(handle), libvcx_error::SUCCESS.code_num);
