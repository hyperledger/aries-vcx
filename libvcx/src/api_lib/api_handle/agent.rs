...
pub async fn create_public_agent(source_id: &str, institution_did: &str) -> VcxResult<u32> {
    ...
    let agent = PublicAgent::create(get_main_wallet_handle(), &get_main_agency_client().unwrap(), source_id, institution_did).await?;
...
pub async fn download_connection_requests(handle: u32, uids: Option<&Vec<String>>) -> VcxResult<String> {
    ...
    let requests = agent.download_connection_requests(&get_main_agency_client().unwrap(), uids.map(|v| v.clone())).await?;
