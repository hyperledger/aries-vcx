...
pub async fn create_and_publish_schema(
    ...
    let schema = Schema::create(source_id, &issuer_did, &name, &version, &data)
        .await?
        .publish(get_main_wallet_handle(), get_main_pool_handle()?, None)
        .await?;
...
pub async fn prepare_schema_for_endorser(
    ...
    let schema_json = schema
        .get_schema_json(get_main_wallet_handle(), get_main_pool_handle()?)
        .await?;
