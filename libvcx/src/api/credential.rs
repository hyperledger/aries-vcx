...
/// Error code as a u32
#[no_mangle]
#[deprecated(since = "0.15.0", note = "Use vcx_credential_update_state_v2_with_message instead.")]
...
/// Error code as a u32
#[no_mangle]
pub extern fn vcx_credential_update_state_v2_with_message(command_handle: CommandHandle,
                                                       credential_handle: u32,
                                                       connection_handle: u32,
                                                       message: *const c_char,
                                                       cb: Option<extern fn(xcommand_handle: CommandHandle, err: u32, state: u32)>) -> u32 {
    info!("vcx_credential_update_state_v2_with_message >>>");
    ...
    let source_id = credential::get_source_id(credential_handle).unwrap_or_default();
    trace!("vcx_credential_update_state_v2_with_message(command_handle: {}, credential_handle: {}), source_id: {:?}",
           command_handle, credential_handle, source_id);
    ...
    spawn(move || {
        match credential::update_state(credential_handle, Some(&message), Some(connection_handle)) {
            Ok(_) => (),
            Err(e) => {
                error!("vcx_credential_update_state_v2_with_message_cb(command_handle: {}, rc: {}, state: {}), source_id: {:?}",
                       command_handle, e, 0, source_id);
                cb(command_handle, e.into(), 0)
            }
        }
        ...
        match credential::get_state(credential_handle) {
            Ok(s) => {
                trace!("vcx_credential_update_state_v2_with_message_cb(command_handle: {}, rc: {}, state: {}), source_id: {:?}",
                       command_handle, error::SUCCESS.message, s, source_id);
                cb(command_handle, error::SUCCESS.code_num, s)
            }
            Err(e) => {
