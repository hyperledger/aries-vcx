...
impl RevocationRegistry {
    ...
    async fn publish_built_rev_reg_delta(&mut self) -> VcxResult<()> {
        ...
        if self.was_rev_reg_delta_published() {
            ...
            self.publish_rev_reg_delta(&issuer_did).await?;
        }
        Ok(())
    }

    async fn publish_built_rev_reg_def(&mut self, tails_url: &str) -> VcxResult<()> {
        let issuer_did = &self.issuer_did.clone();
        if self.was_rev_reg_def_published() {
            info!("No unpublished revocation registry definition found, nothing to publish")
        } else {
            self.publish_rev_reg_def(&issuer_did, tails_url).await?;
        }
        Ok(())
    }
