...
pub mod integration_tests {
    ...
    use crate::libindy::ledger::transactions::{get_cred_def, get_rev_reg, get_rev_reg_def_json, get_rev_reg_delta_json, get_schema_json, is_cred_def_on_ledger};
    use crate::libindy::proofs::prover::prover::libindy_prover_get_credentials_for_proof_req;
    use crate::libindy::primitives::revocation_registry::{generate_rev_reg, libindy_issuer_revoke_credential, publish_local_revocations, revoke_credential_local};
    ...
    async fn test_rev_reg_def_fails_for_cred_def_created_without_revocation() {
        ...
        let (_, _, cred_def_id, _, _) =
            create_and_store_nonrevocable_credential_def(setup.wallet_handle, setup.pool_handle, &setup.institution_did, DEFAULT_SCHEMA_ATTRS)
                .await;
    ...
    async fn test_get_rev_reg_def_json() {
        ...
        let (_, _, _, _, rev_reg_id, _, _) = create_and_store_credential_def(setup.wallet_handle, setup.pool_handle, &setup.institution_did, attrs).await;
    ...
    async fn test_get_cred_def() {
        ...
        let (_, _, cred_def_id, cred_def_json, _) =
            create_and_store_nonrevocable_credential_def(setup.wallet_handle, setup.pool_handle, &setup.institution_did, attrs).await;
