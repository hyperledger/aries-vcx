...
impl BaseLedger for IndySdkLedger {
    ...
    async fn submit_request(&self, request_json: &str) -> VcxCoreResult<String> {
        indy::ledger::transactions::libindy_submit_request(self.indy_pool_handle, request_json)
            .await
            .map_err(|err| err.into())
    }
    ...
    async fn set_endorser(&self, submitter_did: &str, request_json: &str, endorser: &str) -> VcxCoreResult<String> {
        indy::ledger::transactions::set_endorser(self.indy_wallet_handle, submitter_did, request_json, endorser)
            .await
            .map_err(|err| err.into())
    }

    async fn get_txn_author_agreement(&self) -> VcxCoreResult<String> {
        indy::ledger::transactions::libindy_get_txn_author_agreement(self.indy_pool_handle)
            .await
            .map_err(|err| err.into())
    }

    async fn get_nym(&self, did: &str) -> VcxCoreResult<String> {
        indy::ledger::transactions::get_nym(self.indy_pool_handle, did)
            .await
            .map_err(|err| err.into())
    }
    ...
    async fn get_attr(&self, target_did: &str, attr_name: &str) -> VcxCoreResult<String> {
        indy::ledger::transactions::get_attr(self.indy_pool_handle, target_did, attr_name)
            .await
            .map_err(|err| err.into())
    }
    ...
    async fn get_rev_reg_delta_json(
        ...
        indy::ledger::transactions::get_rev_reg_delta_json(self.indy_pool_handle, rev_reg_id, from, to)
            .await
            .map_err(|err| err.into())
    }

    async fn get_rev_reg(&self, rev_reg_id: &str, timestamp: u64) -> VcxCoreResult<(String, String, u64)> {
        indy::ledger::transactions::get_rev_reg(self.indy_pool_handle, rev_reg_id, timestamp)
            .await
            .map_err(|err| err.into())
    }
    ...
    async fn build_schema_request(&self, submitter_did: &str, schema_json: &str) -> VcxCoreResult<String> {
        indy::ledger::transactions::build_schema_request(submitter_did, schema_json)
            .await
            .map_err(|err| err.into())
    }
