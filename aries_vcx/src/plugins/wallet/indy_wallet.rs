...
impl BaseWallet for IndySdkWallet {
    async fn create_and_store_my_did(
        ...
        indy::keys::create_and_store_my_did(self.wallet_handle, seed, method_name)
            .await
            .map_err(|err| err.into())
    }

    async fn key_for_local_did(&self, did: &str) -> VcxCoreResult<String> {
        indy::keys::get_verkey_from_wallet(self.wallet_handle, did)
            .await
            .map_err(|err| err.into())
    }

    async fn replace_did_keys_start(&self, target_did: &str) -> VcxCoreResult<String> {
        indy::keys::libindy_replace_keys_start(self.wallet_handle, target_did)
            .await
            .map_err(|err| err.into())
    }

    async fn replace_did_keys_apply(&self, target_did: &str) -> VcxCoreResult<()> {
        indy::keys::libindy_replace_keys_apply(self.wallet_handle, target_did)
            .await
            .map_err(|err| err.into())
    }
    ...
    async fn add_wallet_record(
        ...
        indy::wallet::add_wallet_record(self.wallet_handle, xtype, id, value, tags_json)
            .await
            .map_err(|err| err.into())
    }

    async fn get_wallet_record(&self, xtype: &str, id: &str, options_json: &str) -> VcxCoreResult<String> {
        indy::wallet::get_wallet_record(self.wallet_handle, xtype, id, options_json)
            .await
            .map_err(|err| err.into())
    }

    async fn delete_wallet_record(&self, xtype: &str, id: &str) -> VcxCoreResult<()> {
        indy::wallet::delete_wallet_record(self.wallet_handle, xtype, id)
            .await
            .map_err(|err| err.into())
    }

    async fn update_wallet_record_value(&self, xtype: &str, id: &str, value: &str) -> VcxCoreResult<()> {
        indy::wallet::update_wallet_record_value(self.wallet_handle, xtype, id, value)
            .await
            .map_err(|err| err.into())
    }

    async fn update_wallet_record_tags(&self, xtype: &str, id: &str, tags_json: &str) -> VcxCoreResult<()> {
        indy::wallet::update_wallet_record_tags(self.wallet_handle, xtype, id, tags_json)
            .await
            .map_err(|err| err.into())
    }

    async fn add_wallet_record_tags(&self, xtype: &str, id: &str, tags_json: &str) -> VcxCoreResult<()> {
        indy::wallet::add_wallet_record_tags(self.wallet_handle, xtype, id, tags_json)
            .await
            .map_err(|err| err.into())
    }

    async fn delete_wallet_record_tags(&self, xtype: &str, id: &str, tag_names: &str) -> VcxCoreResult<()> {
        indy::wallet::delete_wallet_record_tags(self.wallet_handle, xtype, id, tag_names)
            .await
            .map_err(|err| err.into())
    }
