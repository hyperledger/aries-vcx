    ...
    pub fn get_credential(&self) -> VcxResult<(String, A2AMessage)> {
        match self.state {
            HolderFullState::Finished(ref state) => {
                let cred_id = state.cred_id.clone().ok_or(ErrorAriesVcx::from_msg(
                    ErrorKindAriesVcx::InvalidState,
                    "Cannot get credential: Credential Id not found",
                ))?;
                let credential = state.credential.clone().ok_or(ErrorAriesVcx::from_msg(
                    ErrorKindAriesVcx::InvalidState,
...
mod test {
    ...
    mod get_tails_location {
        ...
        async fn test_get_tails_location() {
            ...
            assert_eq!(
                Err(ErrorKindAriesVcx::NotReady),
