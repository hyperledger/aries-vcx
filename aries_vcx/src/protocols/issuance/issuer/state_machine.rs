...
impl IssuerSM {
    ...
    pub fn get_rev_id(&self) -> VcxResult<String> {
        let err = ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidState, "No revocation info found - is this credential revokable?");
        ...
        rev_id.ok_or(ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidState, "Revocation info does not contain rev id"))
    }
    ...
    pub fn get_rev_reg_id(&self) -> VcxResult<String> {
        ...
        rev_registry.ok_or(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::InvalidState,
    ...
    pub async fn is_revoked(&self, profile: &Arc<dyn Profile>) -> VcxResult<bool> {
        if self.is_revokable() {
            ...
            Err(ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidState, "Unable to check revocation status - this credential is not revokable"))
        }
    }
    ...
    pub async fn send_credential_offer(self, send_message: SendClosure) -> VcxResult<Self> {
        Ok(match self.state {
            ...
            _ => { return Err(ErrorAriesVcx::from_msg(ErrorKindAriesVcx::NotReady, "Invalid action")); }
        })
    }
