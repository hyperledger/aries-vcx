...
impl InviterConnection<Initial> {
    ...
    pub fn into_invited_by_request(self, msg_request: &Request) -> InviterConnection<Invited> {
        let thread_id = msg_request
            .decorators
            .thread
            .as_ref()
            .map(|t| t.thid.as_str())
...
impl InviterConnection<Invited> {
    ...
    ///
    /// NOTE: Calling [`InviterConnection<Initial>::into_invited_by_request()`]
