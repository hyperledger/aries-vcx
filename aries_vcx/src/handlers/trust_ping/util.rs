...
pub(super) fn build_ping_response(ping: &Ping) -> PingResponse {
    let thread_id = ping.thread.as_ref()
        ...
        T: Future<Output=VcxResult<()>>
{
    if ping.response_requested {
        send_message(wallet_handle, pw_vk.to_string(), did_doc.clone(), build_ping_response(ping).to_a2a_message()).await?;
    }
    Ok(())
}
...
pub mod unit_tests {
    ...
    fn test_build_ping_response_works() {
        assert_eq!(build_ping_response(&_ping()).get_thread_id(), _ping_response().get_thread_id());
        assert_eq!(build_ping_response(&_ping_no_thread()).get_thread_id(), _ping_response().get_thread_id());
        assert_eq!(build_ping_response(&_ping_no_thread()).get_thread_id(), _ping_no_thread().id.0);
    }
}
