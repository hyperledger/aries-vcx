...
use crate::indy::ledger::transactions::{_check_schema_response, build_schema_request, get_schema_json, sign_and_submit_to_ledger, self};
...
impl Schema {
    ...
    pub async fn publish(self, wallet_handle: WalletHandle, pool_handle: PoolHandle, endorser_did: Option<String>) -> VcxResult<Self> {
        ...
        if let Some(endorser_did) = endorser_did {
            request = transactions::set_endorser(wallet_handle, &self.submitter_did, &request, &endorser_did).await?;
        }
