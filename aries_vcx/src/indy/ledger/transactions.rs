...
fn parse_response(response: &str) -> VcxResult<Response> {
    serde_json::from_str::<Response>(response)
        .map_err(|err| ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidJson, format!("Cannot deserialize object: {}", err)))
}
...
pub async fn endorse_transaction(wallet_handle: WalletHandle, pool_handle: PoolHandle, endorser_did: &str, transaction_json: &str) -> VcxResult<()> {
    ...
    match parse_response(&response)? {
        Response::Reply(_) => Ok(()),
        Response::Reject(res) | Response::ReqNACK(res) => Err(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::PostMessageFailed,
            format!("{:?}", res.reason),
        )),
    }
}
...
fn _verify_transaction_can_be_endorsed(transaction_json: &str, _did: &str) -> VcxResult<()> {
    let transaction: Request = serde_json::from_str(transaction_json)
        .map_err(|err| ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidJson, format!("{:?}", err)))?;

    let transaction_endorser = transaction.endorser.ok_or(ErrorAriesVcx::from_msg(
...
pub async fn is_cred_def_on_ledger(pool_handle: PoolHandle, issuer_did: Option<&str>, cred_def_id: &str) -> VcxResult<bool> {
    match get_cred_def(pool_handle, issuer_did, cred_def_id).await {
        Ok(_) => Ok(true),
        Err(err) if err.kind() == ErrorKindAriesVcx::LibndyError(309) => Ok(false),
        Err(err) => Err(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::InvalidLedgerResponse,
            format!(
...
pub fn _check_schema_response(response: &str) -> VcxResult<()> {
    ...
    match parse_response(response)? {
        Response::Reply(_) => Ok(()),
        Response::Reject(reject) => Err(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::DuplicationSchema,
            format!("{:?}", reject),
        )),
        Response::ReqNACK(reqnack) => Err(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::UnknownSchemaRejection,
            format!("{:?}", reqnack),
        )),
    }
}
