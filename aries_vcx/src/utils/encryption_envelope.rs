...
impl EncryptionEnvelope {
    pub async fn create(wallet_handle: WalletHandle,
                        message: &A2AMessage,
    ...
    async fn wrap_into_forward(wallet_handle: WalletHandle,
                               message: Vec<u8>,
                               to: &str,
    ...
    pub async fn auth_unpack(wallet_handle: WalletHandle, payload: Vec<u8>, expected_vk: &str) -> VcxResult<A2AMessage> {
...
pub mod unit_tests {
    ...
    async fn test_encryption_envelope_works_for_no_keys() {
        ...
        let res = EncryptionEnvelope::create(setup.wallet_handle, &message, Some(&trustee_key), &DidDoc::default()).await;
    ...
    async fn test_encryption_envelope_works_for_routing_keys() {
        ...
        let envelope = EncryptionEnvelope::create(setup.wallet_handle, &ack, Some(&trustee_key), &did_doc).await.unwrap();

        let message_1 = EncryptionEnvelope::anon_unpack(setup.wallet_handle, envelope.0).await.unwrap();
        ...
        let message_2 = EncryptionEnvelope::anon_unpack(setup.wallet_handle, message_1).await.unwrap();
        ...
        assert_eq!(ack, EncryptionEnvelope::anon_unpack(setup.wallet_handle, message_2).await.unwrap());
    }
