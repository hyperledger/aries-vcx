    ...
    if let Value::Object(mut map) = val {
        ...
        while let Some(k) = keys.pop() {
            let mut value = map.remove(&k).ok_or_else(|| {
                ...
                ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidJson, "Unexpected key value mutation")
            })?;
...
pub(crate) trait TryGetIndex {
    type Val;
    fn try_get(&self, index: &str) -> Result<Self::Val, ErrorAriesVcx>;
}

impl<'a> TryGetIndex for &'a Value {
    type Val = &'a Value;
    fn try_get(&self, index: &str) -> Result<&'a Value, ErrorAriesVcx> {
        self.get(index).ok_or(ErrorAriesVcx::from_msg(
...
pub(crate) trait AsTypeOrDeserializationError {
    fn try_as_str(&self) -> Result<&str, ErrorAriesVcx>;

    fn try_as_object(&self) -> Result<&Map<String, Value>, ErrorAriesVcx>;

    fn try_as_bool(&self) -> Result<bool, ErrorAriesVcx>;

    fn try_as_array(&self) -> Result<&Vec<Value>, ErrorAriesVcx>;
}

impl AsTypeOrDeserializationError for &Value {
    fn try_as_str(&self) -> Result<&str, ErrorAriesVcx> {
        self.as_str().ok_or(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::InvalidJson,
    ...
    fn try_as_object(&self) -> Result<&Map<String, Value>, ErrorAriesVcx> {
        self.as_object().ok_or(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::InvalidJson,
    ...
    fn try_as_bool(&self) -> Result<bool, ErrorAriesVcx> {
        self.as_bool().ok_or(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::InvalidJson,
    ...
    fn try_as_array(&self) -> Result<&Vec<Value>, ErrorAriesVcx> {
        self.as_array().ok_or(ErrorAriesVcx::from_msg(
            ErrorKindAriesVcx::InvalidJson,
            format!("Could not deserialize '{}' value as bool", self.to_string()),
        ))
    }
