...
pub fn validate_nonce(nonce: &str) -> VcxResult<String> {
    let nonce = BigNum::from_dec_str(nonce).map_err(|err| ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidNonce, err))?;
    if nonce.num_bits() > 80 {
        return Err(ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidNonce, "Invalid Nonce length"));
    }
    Ok(nonce.to_string())
}
...
pub fn validate_actors(actors: &str) -> VcxResult<Vec<Actors>> {
    ::serde_json::from_str(actors)
        .map_err(|err| ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidOption, format!("Invalid actors: {:?}", err)))
}
...
mod unit_tests {
    ...
    fn test_did_is_b58_but_invalid_length() {
        ...
        match validate_did(&to_did) {
            Err(x) => assert_eq!(x.kind(), ErrorKindAriesVcx::InvalidDid),
    ...
    fn test_validate_did_with_non_base58() {
        ...
        match validate_did(&to_did) {
            Err(x) => assert_eq!(x.kind(), ErrorKindAriesVcx::NotBase58),
            Ok(_) => panic!("Should be invalid did"),
        }
    }
}
