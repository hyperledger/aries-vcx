...
use crate::errors::error::{ErrorAriesVcx, VcxResult, ErrorKindAriesVcx};
...
impl Schema {
    pub async fn create(profile: &Arc<dyn Profile>, source_id: &str, submitter_did: &str, name: &str, version: &str, data: &Vec<String>) -> VcxResult<Self> {
        ...
        .map_err(|err| ErrorAriesVcx::from_msg(ErrorKindAriesVcx::SerializationError, format!("Failed to serialize schema attributes, err: {}", err)))?;
    ...
    pub async fn create_from_ledger_json(profile: &Arc<dyn Profile>, source_id: &str, schema_id: &str) -> VcxResult<Self> {
        ...
        let schema_data: SchemaData = serde_json::from_str(&schema_json)
            .map_err(|err| ErrorAriesVcx::from_msg(ErrorKindAriesVcx::InvalidJson, format!("Cannot deserialize schema: {}", err)))?;
    ...
    pub fn to_string(&self) -> VcxResult<String> {
        ObjectWithVersion::new(DEFAULT_SERIALIZE_VERSION, self.to_owned())
            .serialize()
            .map_err(|err| err)
            .map_err(|err: ErrorAriesVcx| err.extend("Cannot serialize Schema"))
    }
    ...
    pub fn from_str(data: &str) -> VcxResult<Schema> {
        ObjectWithVersion::deserialize(data)
            ...
            .map_err(|err| err)
            .map_err(|err: ErrorAriesVcx| err.extend("Cannot deserialize Schema"))
    }
