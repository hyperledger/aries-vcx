...
pub mod integration_tests {
    ...
    async fn tests_returns_error_if_proof_request_is_malformed() {
        ...
        assert_eq!(result.unwrap_err().kind(), ErrorKindAriesVcx::InvalidProofRequest);
        }).await;
    }
    ...
    async fn tests_prover_get_credentials() {
        ...
        let anoncreds = Arc::clone(&setup.profile).inject_anoncreds();
        let _result = anoncreds.prover_get_credentials_for_proof_req(&proof_req).await.unwrap();

        let result_malformed_json = anoncreds.prover_get_credentials_for_proof_req("{}")
            .await
            .unwrap_err();
        assert_eq!(result_malformed_json.kind(), ErrorKindAriesVcx::InvalidAttributesStructure);
        }).await;
    }
    ...
    async fn test_revoke_credential() {
        ...
        anoncreds.revoke_credential_local(
        ...
        anoncreds.publish_local_revocations(
            &setup.institution_did,
            &rev_reg_id,
        )
        .await
        ...
        let (_, second_rev_reg_delta, _) =
            ledger.get_rev_reg_delta_json(&rev_reg_id, Some(first_timestamp + 1), None)
                .await
                .unwrap();
