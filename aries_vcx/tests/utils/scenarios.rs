...
pub mod test_utils {
    ...
    pub async fn create_connected_connections_via_public_invite(
        ...
        let mut consumer_to_institution =
            Connection::create_with_invite("institution", &alice.profile, &alice.agency_client, public_invite, ddo, true)
                .await
                .unwrap();
        consumer_to_institution
    ...
    pub async fn create_connected_connections(alice: &mut Alice, faber: &mut Faber) -> (Connection, Connection) {
        ...
        let mut institution_to_consumer = Connection::create("consumer", &faber.profile, &faber.agency_client, true)
            .await
            .unwrap();
        institution_to_consumer
        ...
        let mut consumer_to_institution =
            Connection::create_with_invite("institution", &alice.profile, &alice.agency_client, details.clone(), ddo, true)
                .await
                .unwrap();
