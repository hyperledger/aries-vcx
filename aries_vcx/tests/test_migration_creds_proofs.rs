...
mod integration_tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_retrieve_credentials() {
        SetupProfile::run(|mut setup: SetupProfile| async move {
            ...
            let new_wh = crate::utils::migration::migrate_and_replace_profile(setup.profile.wallet_handle().unwrap()).await;
            setup.profile = crate::utils::migration::make_modular_profile(new_wh);
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_get_credential_def() {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_retrieve_credentials_empty() {
        SetupProfile::run(|mut setup| async move {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_case_for_proof_req_doesnt_matter_for_retrieve_creds() {
        SetupProfile::run(|mut setup| async move {
            create_and_store_nonrevocable_credential(
                &setup.profile.inject_anoncreds(),
                &setup.profile.inject_anoncreds(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_generate_proof() {
        SetupProfile::run(|mut setup| async move {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_generate_proof_with_predicates() {
        SetupProfile::run(|mut setup| async move {
            create_and_store_credential(
                &setup.profile.inject_anoncreds(),
                &setup.profile.inject_anoncreds(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_generate_self_attested_proof() {
        SetupProfile::run(|mut setup| async move {
            let indy_proof_req = json!({
               "nonce":"123432421212",
               "name":"proof_req_1",
...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_proof_should_be_validated() {
        SetupPool::run(|setup| async move {
            ...
            let (schema_id, cred_def_id, _rev_reg_id, _cred_def, _rev_reg, _credential_handle) =
                ...
                let new_wh = crate::utils::migration::migrate_and_replace_profile(institution.profile.wallet_handle().unwrap()).await;
                institution.profile = crate::utils::migration::make_modular_profile(new_wh);
                let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
                institution.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
            ...
            let new_wh = crate::utils::migration::migrate_and_replace_profile(consumer.profile.wallet_handle().unwrap()).await;
            consumer.profile = crate::utils::migration::make_modular_profile(new_wh);
            let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
            consumer.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_proof_with_predicates_should_be_validated() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_it_should_fail_to_select_credentials_for_predicate() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_double_issuance_separate_issuer_and_consumers() {
        SetupPool::run(|setup| async move {
            ...
            let (consumer2_to_issuer, issuer_to_consumer2) =
                ...
                let new_wh = crate::utils::migration::migrate_and_replace_profile(issuer.profile.wallet_handle().unwrap()).await;
            issuer.profile = crate::utils::migration::make_modular_profile(new_wh);
            let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
            issuer.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
            ...
            let _credential_handle1 = _exchange_credential(
                ...
                let new_wh = crate::utils::migration::migrate_and_replace_profile(verifier.profile.wallet_handle().unwrap()).await;
            verifier.profile = crate::utils::migration::make_modular_profile(new_wh);
            let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
            verifier.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
            ...
            let mut proof_verifier = verifier_create_proof_and_send_request(
                ...
                let new_wh = crate::utils::migration::migrate_and_replace_profile(consumer1.profile.wallet_handle().unwrap()).await;
            consumer1.profile = crate::utils::migration::make_modular_profile(new_wh);
            let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
            consumer1.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
            ...
            let mut proof_verifier = verifier_create_proof_and_send_request(
                ...
                let new_wh = crate::utils::migration::migrate_and_replace_profile(consumer2.profile.wallet_handle().unwrap()).await;
                consumer2.profile = crate::utils::migration::make_modular_profile(new_wh);
                let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
                consumer2.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_double_issuance_separate_issuer() {
        SetupPool::run(|setup| async move {
            let mut issuer = Faber::setup(setup.pool_handle).await;
            let mut verifier = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_double_issuance_issuer_is_verifier() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_real_proof() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_two_creds_one_rev_reg() {
        SetupPool::run(|setup| async move {
            let mut issuer = Faber::setup(setup.pool_handle).await;
            let mut verifier = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    pub async fn test_migration_agency_pool_credential_exchange_via_proposal() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    pub async fn test_migration_agency_pool_credential_exchange_via_proposal_failed() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    pub async fn test_migration_agency_pool_credential_exchange_via_proposal_with_negotiation() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    pub async fn test_migration_agency_pool_presentation_via_proposal() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    pub async fn test_migration_agency_pool_presentation_via_proposal_with_rejection() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    pub async fn test_migration_agency_pool_presentation_via_proposal_with_negotiation() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_aries_demo() {
        ...
        SetupPool::run(|pool| async move {
            ...
            let new_wh = crate::utils::migration::migrate_and_replace_profile(faber.profile.wallet_handle().unwrap()).await;
            faber.profile = crate::utils::migration::make_modular_profile(new_wh);
            let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
            faber.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
            ...
            let new_wh = crate::utils::migration::migrate_and_replace_profile(alice.profile.wallet_handle().unwrap()).await;
            alice.profile = crate::utils::migration::make_modular_profile(new_wh);
            let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
            alice.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_aries_demo_create_with_message_id_flow() {
        let _setup = SetupEmpty::init();
        SetupPool::run(|pool| async move {
            let mut faber = Faber::setup(pool.pool_handle).await;
            let mut alice = create_test_alice_instance(&pool).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_aries_demo_download_message_flow() {
        SetupEmpty::init();
        SetupPool::run(|pool| async move {
            let mut faber = Faber::setup(pool.pool_handle).await;
            let mut alice = create_test_alice_instance(&pool).await;
