...
mod integration_tests {
    ...
    async fn test_rotate_verkey() {
        ...
        rotate_verkey(setup.wallet_handle, setup.pool_handle, &did)
            .await
            .unwrap();
    ...
    async fn test_endorse_transaction() {
        ...
        endorse_transaction(setup.wallet_handle, setup.pool_handle, &endorser_did, &schema_request)
            .await
            .unwrap();
    }
    ...
    async fn test_add_get_service() {
        ...
        add_service(setup.wallet_handle, setup.pool_handle, &did, &expect_service)
            .await
            .unwrap();
