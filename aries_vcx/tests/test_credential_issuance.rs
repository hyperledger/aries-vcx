...
#[tokio::test]
#[ignore]
async fn test_agency_pool_double_issuance_issuer_is_verifier() -> Result<(), Box<dyn Error>> {
...
#[ignore]
#[allow(unused_mut)]
async fn test_agency_pool_two_creds_one_rev_reg() -> Result<(), Box<dyn Error>> {
    let setup = SetupPoolDirectory::init().await;
...
#[ignore]
#[allow(unused_mut)]
async fn test_agency_pool_credential_exchange_via_proposal() -> Result<(), Box<dyn Error>> {
    let setup = SetupPoolDirectory::init().await;
    let mut institution = create_test_agent_trustee(setup.genesis_file_path.clone()).await;
    let mut consumer = create_test_agent(setup.genesis_file_path).await;
...
#[ignore]
#[allow(unused_mut)]
async fn test_agency_pool_credential_exchange_via_proposal_failed() -> Result<(), Box<dyn Error>> {
    let setup = SetupPoolDirectory::init().await;
    let mut institution = create_test_agent_trustee(setup.genesis_file_path.clone()).await;
    let mut consumer = create_test_agent(setup.genesis_file_path.clone()).await;
    ...
    issuer.process_aries_msg(problem_report.into()).await?;
