...
mod integration_tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_basic_revocation() {
        SetupPool::run(|setup| async move {
            ...
            let new_wh =
                crate::utils::migration::migrate_and_replace_profile(institution.profile.wallet_handle().unwrap())
                    .await;
            ...
            let new_wh =
                crate::utils::migration::migrate_and_replace_profile(consumer.profile.wallet_handle().unwrap()).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_revocation_notification() {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_local_revocation() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_batch_revocation() {
        SetupPool::run(|setup| async move {
        let mut institution = Faber::setup(setup.pool_handle).await;
...
let new_wh =
    crate::utils::migration::migrate_and_replace_profile(consumer1.profile.wallet_handle().unwrap()).await;
...
assert!(!issuer_credential3.is_revoked(&institution.profile.inject_anoncreds_ledger_read()).await.unwrap());
        ...
        let new_wh =
        crate::utils::migration::migrate_and_replace_profile(consumer2.profile.wallet_handle().unwrap())
            .await;
    ...
    let new_wh =
        crate::utils::migration::migrate_and_replace_profile(consumer3.profile.wallet_handle().unwrap()).await;
    consumer3.profile = crate::utils::migration::make_modular_profile(new_wh);
    let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
    consumer3.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_revoked_credential_might_still_work() {
        SetupPool::run(|setup| async move {
            let mut institution = Faber::setup(setup.pool_handle).await;
            let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_two_creds_one_rev_reg_revoke_first() {
        SetupPool::run(|setup| async move {
        ...
        let new_wh =
        crate::utils::migration::migrate_and_replace_profile(issuer.profile.wallet_handle().unwrap())
            .await;
...
let new_wh =
    crate::utils::migration::migrate_and_replace_profile(verifier.profile.wallet_handle().unwrap()).await;
...
let mut proof_verifier = verifier_create_proof_and_send_request(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_two_creds_one_rev_reg_revoke_second() {
        SetupPool::run(|setup| async move {
        let mut issuer = Faber::setup(setup.pool_handle).await;
        let mut verifier = Faber::setup(setup.pool_handle).await;
        let mut consumer = create_test_alice_instance(&setup).await;
        ...
        let new_wh =
        crate::utils::migration::migrate_and_replace_profile(issuer.profile.wallet_handle().unwrap()).await;
    issuer.profile = crate::utils::migration::make_modular_profile(new_wh);
    let new_wallet: Arc<dyn BaseWallet> = Arc::new(IndySdkWallet::new(new_wh));
    issuer.agency_client.wallet = new_wallet.to_base_agency_client_wallet();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_two_creds_two_rev_reg_id() {
        SetupPool::run(|setup| async move {
        let mut issuer = Faber::setup(setup.pool_handle).await;
        let mut verifier = Faber::setup(setup.pool_handle).await;
        let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_two_creds_two_rev_reg_id_revoke_first() {
        SetupPool::run(|setup| async move {
        let mut issuer = Faber::setup(setup.pool_handle).await;
        let mut verifier = Faber::setup(setup.pool_handle).await;
        let mut consumer = create_test_alice_instance(&setup).await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_migration_agency_pool_two_creds_two_rev_reg_id_revoke_second() {
        SetupPool::run(|setup| async move {
        let mut issuer = Faber::setup(setup.pool_handle).await;
        let mut verifier = Faber::setup(setup.pool_handle).await;
        let mut consumer = create_test_alice_instance(&setup).await;
