...
module.exports.createServiceCredIssuer = function createServiceCredIssuer(logger, loadConnection, loadCredDef, storeCredential, loadCredential) {
  ...
  async function sendOffer ({ issuerCredName, schemaAttrs, credDefName, connectionName }) {
    ...
    let serIssuerCred = await issuerCred.serialize()
  ...
  async function sendCredential ({ issuerCredName, connectionName }) {
    ...
    let serIssuerCredAfter = await issuerCred.serialize()
  ...
  async function sendOfferAndWaitForCredRequest ({ issuerCredName, connectionName, credDefName, schemaAttrs }) {
    ...
    let issuerCred = await sendOffer({ issuerCredName, schemaAttrs, credDefName, connectionName })
    ...
    let serIssuerCred2 = await issuerCred.serialize()
